...
export function ScrollVideoRevealSection({ tracks }: { tracks: LearningTrack[] }) {
  ...
  useEffect(() => {
    const update = () => {
      ...
      if (duration > 0) {
        ...
        const targetTime = clamp(videoProgress * duration, 0, Math.max(duration - 0.01, 0));
  ...
  const videoScale = 1 + Math.min(progress, 0.78) * 0.035;
  ...
  return (
      <section ref={sectionRef} className="relative h-[260vh] md:h-[460vh]">      <div className={pinClassName}>
        <div className="relative h-full w-full overflow-hidden bg-slate-950">
          ...
          <div className="absolute inset-0 flex items-end justify-center px-4 pb-10 md:px-8 md:pb-16 pointer-events-none">
            <div
            ...
            >
              <div className="mx-auto max-w-5xl rounded-[40px] border border-white/10 bg-slate-950/72 px-5 py-6 shadow-[0_30px_90px_rgba(2,6,23,0.5)] backdrop-blur-xl md:px-10 md:py-10">
                <div className="inline-flex rounded-full border border-cyan-400/20 bg-cyan-400/10 px-4 py-2 text-sm text-cyan-200">
                  מה נלמד בפועל
                </div>

                <h2 className="mt-6 text-[2.45rem] font-semibold leading-[1.06] text-white md:text-[3rem] lg:text-[3.2rem]">
                  ארכיטקטורת ה-AI שניישם בעסק שלך
                </h2>

                <p className="mt-5 max-w-4xl text-base leading-8 text-slate-200 md:text-lg">
                  התהליך הזה לא בנוי לטריקים מגניבים ולא לעוד רשימת כלים מבלבלת. אנחנו בונים תשתית עבודה
                  חכמה לעצמאי שמחזיק את העסק על הכתפיים שלו, כדי לחתוך עלויות, לקצר זמני הפקה ולהוציא יותר
                  תוכן בפחות מאמץ.
                </p>
                ...
                <div className="mt-8 grid gap-5 md:grid-cols-2">
                  {tracks.map((track, index) => (
                    <div
                      key={track.title}
                      className="rounded-[30px] border border-white/10 bg-white/5 p-5 shadow-[0_18px_55px_rgba(2,6,23,0.24)] backdrop-blur-xl md:p-6"
                      style={{
                        opacity: textOpacity,
                        transform: `translateY(${Math.max(0, 18 - textOpacity * 18 + index * 2)}px)`
                      }}
                    >
                      ...
                      <p className="mt-4 text-sm leading-7 text-slate-200 md:text-base md:leading-8">{track.text}</p>
                    </div>
                  ))}
                </div>
              </div>
